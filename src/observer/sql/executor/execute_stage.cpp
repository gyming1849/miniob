...
=======
				 http://license.coscl.org.cn/MulanPSL2
>>>>>>> 2e205cee9b71d29fe6a8e7eb60a116dd74419886
		...
		for(int j=0;j<selects.relation_num;j++) {
			if (strcmp(attr.relation_name, selects.relations[j])==0) {
				if(strcmp("*",attr.relation_name)==0) {
					...
					schema_add_field(tables[j], attr.attribute_name, attr.aggregation_type, schema_result);
      }
				}
			}
		}
...
RC ExecuteStage::init_select(const char *db, const Selects &selects, Table **tables, TupleSchema &schema_result) {
...
static RC schema_add_field(Table *table, const char *field_name, AggregationFunc aggregation_type, TupleSchema &schema) {
